...
//! are defined
//vector<Magnetic_object*> Magnetic_object_list;
...
namespace cs251
{
  ...
  void callbacks_t::keyboard_cb(unsigned char key, int x, int y)
  {
    ...
    case 'r':
      {
      vector<Magnetic_object*>::iterator a1 = Magnetic_object_list.begin();   //iterating vector of magnetic object
      for(;a1!=Magnetic_object_list.end();a1++)
      {
            (*a1)->power=1;                     //changing to attractive nature
      }
      delete test;
    ...
    case 'Q':								//event when Q is pressed
    {
        vector<Magnetic_object*>::iterator a1 = Magnetic_object_list.begin();		//iterating vector of magnetic object
        for(;a1!=Magnetic_object_list.end();a1++)
        {
    ...
    case 'T':														//event when T is pressed
        {
        vector<Magnetic_object*>::iterator a1 = Magnetic_object_list.begin();		//iterating vector of magnetic object
        for(;a1!=Magnetic_object_list.end();a1++)					
        {
            (*a1)->change_nature();									//Reversing the magnetic nature							
        }
        break;
        }
  ...
  void callbacks_t::mouse_cb(int32 button, int32 state, int32 x, int32 y)
  {
	...
	if (state == GLUT_UP)						//if mouse is clicked
	  {
      if(x<230 && x>200 && y<192 && y>160)					
        Magnetic_object_list[0]->change_nature();			//change magnetic nature if coordinates are between those given
      if(x<442 && x>410 && y<192 && y>160)
        Magnetic_object_list[1]->change_nature();			//change magnetic nature if coordinates are between those given
	    test->mouse_up(p);
	  }
      }
