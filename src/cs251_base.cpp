...
vector<Magnetic_object*> Magnetic_object_list;                //include vector from structure.hpp
vector<Non_magnetic_object*> Non_Magnetic_object_list;
...
void base_sim_t::step(settings_t* settings)
{
  ...
  m_world->Step(time_step, settings->velocity_iterations, settings->position_iterations);	
      ...
      b2Vec2 delta;
      float r;
      vector<Non_magnetic_object*>::iterator a2 = Non_Magnetic_object_list.begin(); //iterator for non-magnetic material list
        for(;a2!=Non_Magnetic_object_list.end();a2++)                               //looping through non-magnetic material
        {
          b2Vec2 force;                                                           
          force(0)=0;                       
          force(1)=0;
            vector<Magnetic_object*>::iterator a1 = Magnetic_object_list.begin();  //iterator for magnetic material list   
            for(;a1!=Magnetic_object_list.end();a1++)
            {
                delta=(*a2)->body->GetPosition()-(*a1)->body->GetPosition();        //get position vector of non-magnetic wrt to magnetic material
                r=delta.Length();
                force+=-(10000.0/r/r/r*((*a1)->power))*delta;                       //vector addition of force
            }
            (*a2)->body->ApplyForce( force,(*a2)->body->GetPosition(),wake);        //applying force on a object
        }
